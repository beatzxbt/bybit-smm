...
class MarketMaker:
    ...
    def __init__(self, sharedstate) -> None:
        ...
        self.bybit_mark = self.ss.bybit_mark_price
    ...
    def quotes_price_range(self) -> np.array:
        ...
        if self.bybit_mark - self.ss.target_spread > best_ask_price:
            best_bid_price = best_ask_price - self.ss.bybit_tick_size

        else:
            best_bid_price = self.bybit_mark - self.ss.target_spread

        # Same as above, but for asks \
        if self.bybit_mark + self.ss.target_spread < best_bid_price:
            best_ask_price = best_bid_price + self.ss.bybit_tick_size    

        else:
            best_ask_price = self.bybit_mark + self.ss.target_spread

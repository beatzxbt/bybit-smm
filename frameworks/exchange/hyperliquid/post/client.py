...
class HyperLiquidPrivatePostClient:
    ...
    def __init__(self, sharedstate: PrivateDataSharedState) -> None:
        ...
        self.hlq = self.pdss.hyperliquid["API"]
    ...
    def _sign(self, payload: str) -> dict:
        ...
        param_str = "".join([self.timestamp, self.hlq["key"], self.recvWindow, payload])

        header = {
            "X-BAPI-TIMESTAMP": self.timestamp,
            "X-BAPI-API-KEY": self.hlq["key"],
            "X-BAPI-RECV-WINDOW": self.recvWindow,
        }

        hash_signature = hmac.new(
            bytes(self.hlq["secret"], "utf-8"), 
    ...
    def _update_rate_limit(self, endpoint: str, response: json) -> None:
        ...
        if endpoint not in self.hlq:
            max_limit = int(response["X-Bapi-Limit"])
            self.hlq[endpoint] = [remaining_limit, max_limit, time_till_reset]

        else:
            self.hlq[endpoint][0] = remaining_limit
            self.hlq[endpoint][2] = time_till_reset
